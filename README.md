...
- Разработать музыкальную онлайн-платформу с веб-интерфейсом на основе React.

- Спроектировать и реализовать базу данных (PostgreSQL на Supabase) с поддержкой CRUD-операций, связей и ограничений целостности.

- Реализовать серверную логику (PostgreSQL функции, триггеры, представления, транзакции) на уровне базы данных.

- Обеспечить защиту информации (RLS, RBAC, хэширование паролей через Supabase Auth, аудит изменений).

- Создать клиентское приложение на **React (TypeScript)** в стилистике Spotify с имперской цветовой палитрой (чёрный фон, золотые и оранжевые акценты бренда *Imperial Monarchs*).

- Реализовать интеграцию с Supabase API для взаимодействия клиента с базой данных и Storage.

- Разработать импорт/экспорт данных (CSV, JSON, SQL) через веб-интерфейс и автоматизированные скрипты бэкапа.

- Подготовить подсистему аналитики (статистика по трекам, пользователям, плейлистам) с визуализацией через Recharts.

- Провести тестирование (функциональное, интеграционное с Supabase API).
...
- **Dev Tools:** Git, GitHub, Docker, Supabase CLI, Postman

- **Security:** Supabase Auth (хэширование паролей), Row Level Security (RLS), RBAC, аудит изменений через триггеры

- **Analytics:** Recharts (графики и визуализации), SQL-представления для статистики

- **Backup & Recovery:** Node.js скрипты для бэкапа Supabase (база данных + Storage файлы)
