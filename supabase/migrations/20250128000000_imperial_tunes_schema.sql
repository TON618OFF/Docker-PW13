-- Миграция для Supabase на основе ImperialTunesDB1337easy.sql
-- Адаптировано для Supabase с использованием UUID и RLS

-- =================================================================================================
-- СКРИПТ СОЗДАНИЯ БАЗЫ ДАННЫХ IMPERIAL TUNES ДЛЯ SUPABASE
-- Версия: 4.0 (Supabase адаптация)
-- =================================================================================================
...
-- Индексы для таблицы listening_history
...
-- Представление для вычисления длительности альбомов
...
-- Представление для вычисления длительности плейлистов
...
-- Представление для статистики треков
...
-- Функция для ручного создания профиля пользователя
...
BEGIN
  ...
  RETURN jsonb_build_object('success', TRUE, 'message', 'User created successfully');
END;
$$;
...
-- Триггер для создания профиля пользователя
...
-- Триггер для логирования прослушиваний
...
-- Политики для ролей (все могут читать)
...
-- Политики для артистов (все могут читать)
...
-- Политики для жанров (все могут читать)
...
CREATE POLICY "Admins can view all audit logs" ON public.audit_log FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM public.users u
...
-- Storage RLS политики (удаляем существующие и создаем новые)
